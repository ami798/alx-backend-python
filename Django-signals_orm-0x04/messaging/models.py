...
# then use UnreadMessagesManager() in Message definition
...
class UnreadMessagesManager(models.Manager):
    ...
    def unread_for_user(self, user):
        # only necessary fields for performance
        return self.filter(receiver=user, read=False).only("id", "content", "sender", "timestamp")


class Message(models.Model):
    """A user-to-user message, supporting threading, edit tracking, read status, and last editor."""
    sender = models.ForeignKey(User, related_name="sent_messages", on_delete=models.CASCADE)
    receiver = models.ForeignKey(User, related_name="received_messages", on_delete=models.CASCADE)
    content = models.TextField()
    timestamp = models.DateTimeField(auto_now_add=True)
    edited = models.BooleanField(default=False)
    edited_by = models.ForeignKey(  # new field tracking who last edited
